...
def get_model(node_in, hid_dim, num_layers, edge_in=None):
    return LeakDetector(node_in, hid_dim, num_layers, edge_in)
...
def get_data(data_path, edges_directed, logger):
    ...
    test_scenarios = np.random.choice(list(leak_scenarios), TEST_SCENARIOS, replace=False)
    val_scenarios = np.random.choice(list(non_leak_scenarios - set(test_scenarios.tolist())), VAL_SCENARIOS, replace=False)
...
def train(trial_dir, model, train_loader, val_batch, optimizer, scheduler, patience, logger):
    cur_patience = 0
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        logger.info(f'Epoch {epoch:04d}: Train Loss {avg_train_loss:.4f}, Val Loss {val_loss:.4f}. {best}')
        

def test(trial_dir, model, x_test_data, val_batch, test_labels, num_nodes, logger):
    test_batch = next(iter(DataLoader(x_test_data, batch_size=len(x_test_data), shuffle=False)))
    # Load best model
...
def train_and_test(trial, i, study_dir):
    params = {
        k : trial.suggest_categorical(k, HYPERPARAMS[k])
    ...
    train_data, val_data, (test_data, test_labels), (num_nodes, num_edges) = get_data(data_path, edges_directed, logger)
    ...
    model = get_model(node_in=node_dim, edge_in=edge_dim if use_edges else None, hid_dim=hid_dim, num_layers=num_layers)
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=sched_patience)
    
    train_loader = DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True)
    val_batch = next(iter(DataLoader(val_data, batch_size=len(val_data), shuffle=False)))
    
    patience = params["patience"]
    
    train(trial_dir, model, train_loader, val_batch, optimizer, scheduler, patience, logger)
...
def main():
    ...
    scores_df = pd.DataFrame(columns=["Trial"] + [key for key in HYPERPARAMS.keys()] + ["Score"])
